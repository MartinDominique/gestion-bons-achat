...
class DeliveryNotePDFService {
  ...
  addMaterialsTable(doc, deliveryNote, yStart, hideForClient = false) {
    ...
    if (hasAnyBO) {
      columns.push({ header: 'CMD', dataKey: 'ordered' });
      columns.push({ header: 'LIVRÉ', dataKey: 'quantity' });
      columns.push({ header: 'UNITÉ', dataKey: 'unit' });
      columns.push({ header: 'BO', dataKey: 'bo' });
      columnStyles.ordered = { cellWidth: 14, halign: 'center' };
      columnStyles.quantity = { cellWidth: 14, halign: 'center' };
    ...
    if (hasAnyBO && hasBoItems) {
      ...
      doc.text('Items en backorder : un bon de livraison de suivi sera émis.', 15, afterTableY);
