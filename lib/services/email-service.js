...
class ImprovedWorkOrderPDFService {
  ...
  addMaterialsTable(doc, workOrder, yStart, hideForClient = false) {
    ...
    if (shouldShowPrices) {
      colWidths = [15, 70, 15, 15, 25, 30]; // CODE, DESC, QTÉ, UNITÉ, PRIX, TOTAL
      headers = ['CODE', 'DESCRIPTION', 'QTÉ', 'UNITÉ', 'PRIX', 'TOTAL'];
    } else {
      colWidths = [15, 100, 20, 25]; // CODE, DESC, QTÉ, UNITÉ
...
class WorkOrderEmailService {
  ...
  async sendWorkOrderEmail(workOrder, options = {}) {
    try {
      ...
      // ✅ Si prix jobé ET bureau en CC, ajouter le PDF complet
      ...
      if (isPrixJobe) {
        console.log('⚠️ Prix Jobé - 2 PDFs attachés');
      }
