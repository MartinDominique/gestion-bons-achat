// ============================================
// SERVICE EMAIL + PDF - JSPDF V2.5.1 CORRIGÉ
// Fichier: lib/services/email-service.js
// ============================================

const { Resend } = require('resend');
const { jsPDF } = require('jspdf'); // Import correct pour jsPDF v2.x
...
class WorkOrderPDFService {
  ...
  addMaterials(doc, workOrder, y) {
    ...
    workOrder.materials.forEach((material) => {
      ...
      const productName = material.product?.name || material.product?.product_id || 'Matériau sans nom';
  ...
  generatePDF(workOrder) {
    try {
      console.log('Création PDF avec jsPDF v2.5.1...');
      const doc = new jsPDF();
