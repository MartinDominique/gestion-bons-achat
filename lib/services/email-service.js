  ...
  addClientAndWorkSections(doc, workOrder, yStart) {
    ...
    if (workOrder.client.phone) {
      ...
      if (workOrder.time_entries && workOrder.time_entries.length > 0) {
        doc.setFont('helvetica', 'bold');
        doc.text('Sessions de travail:', 110, yRight);
        yRight += 5;
        doc.setFont('helvetica', 'normal');
        ...
        workOrder.time_entries.forEach((entry, index) => {
          // Date
          doc.setFont('helvetica', 'bold');
          doc.text(entry.date, 110, yRight);
          yRight += 4;
          doc.setFont('helvetica', 'normal');
          
          // Heures
          doc.text(`  ${entry.start_time} → ${entry.end_time || 'En cours'}`, 110, yRight);
          yRight += 4;
          
          // Pause si présente
          if (entry.pause_minutes && entry.pause_minutes > 0) {
            doc.text(`  Pause: ${entry.pause_minutes} min`, 110, yRight);
            yRight += 4;
          }
