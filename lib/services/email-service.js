  ...
  addClientAndWorkSections(doc, workOrder, yStart) {
    ...
    if (workOrder.client.phone) {
      ...
      yLeft += 5;
       ...
       // ✅ NOUVEAU - Afficher toutes les sessions
     // ✅ VERSION ULTRA-COMPACTE - 1 LIGNE PAR SESSION
        if (workOrder.time_entries && workOrder.time_entries.length > 0) {
          ...
          workOrder.time_entries.forEach((entry, index) => {
            ...
            yRight += 4; // Seulement 4px par session !
          });
...
class WorkOrderEmailService {
  ...
  async sendWorkOrderEmail(workOrder, options = {}) {
    try {
      // ✅ AJOUT : Email CC automatique depuis COMPANY_EMAIL
      const COMPANY_EMAIL = process.env.COMPANY_EMAIL;
      if (COMPANY_EMAIL && !options.ccEmails) {
        options.ccEmails = [COMPANY_EMAIL];
      } else if (COMPANY_EMAIL && options.ccEmails) {
        options.ccEmails = [...options.ccEmails, COMPANY_EMAIL];
      }
