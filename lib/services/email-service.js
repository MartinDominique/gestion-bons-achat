// ============================================
// SERVICE EMAIL + PDF POUR BONS DE TRAVAIL
...
const COMPANY_EMAIL = process.env.COMPANY_EMAIL || 'votre-email@votreentreprise.com';
const COMPANY_NAME = process.env.COMPANY_NAME || 'Votre Entreprise';
const COMPANY_ADDRESS = process.env.COMPANY_ADDRESS || '123 Rue de l\'Industrie';
const COMPANY_CITY = process.env.COMPANY_CITY || 'Votre Ville, QC';
const COMPANY_PHONE = process.env.COMPANY_PHONE || '(418) 555-1234';
...
class WorkOrderPDFService {
  ...
  addClientInfo(doc, workOrder, y) {
    ...
    doc.text(`Adresse: ${workOrder.client.address}`, 20, y + 15);
  ...
  addWorkDetails(doc, workOrder, y) {
    ...
    doc.text(`Pause: ${workOrder.break_time}h`, 100, y + 8);
  ...
  addMaterials(doc, workOrder, y) {
    ...
    workOrder.materials.forEach((material) => {
      ...
      const productName = doc.splitTextToSize(material.product.name, 115);
      ...
      currentY += productName.length * 4 + 5;
    });
...
<body>
  ...
  async sendWorkOrderEmail(workOrder, options = {}) {
    try {
      // Utiliser l'email du client ou celui fourni en option
      ...
      // Préparer les destinataires CC (bureau + emails supplémentaires)
      const ccEmails = [COMPANY_EMAIL, ...(options.ccEmails || [])];
  ...
  async sendBulkWorkOrders(workOrders, options = {}) {
    const results = [];
    let successCount = 0;
    let failedCount = 0;

    for (const workOrder of workOrders) {
      const result = await this.sendWorkOrderEmail(workOrder, options);
      ...
      // Pause entre envois pour éviter rate limiting
...
function createEmailServices() {
  return {
    pdf: new WorkOrderPDFService(),
    email: new WorkOrderEmailService()
  };
}
