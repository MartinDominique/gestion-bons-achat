...
class WorkOrderPDFService {
  ...
  addMaterials(doc, workOrder, y) {
    ...
    // AUTO-DETECTER si on doit afficher les prix
    // Cherche show_prices au niveau workOrder OU showPrice au niveau matériaux
    const shouldShowPrices = workOrder.show_prices || 
                            (workOrder.materials && workOrder.materials.some(m => m.showPrice === true));
    
    console.log('shouldShowPrices détecté:', shouldShowPrices); // DEBUG
    ...
    workOrder.materials.forEach((material, index) => {
      ...
      // Prix unitaire - CONDITIONNEL selon shouldShowPrices
      if (shouldShowPrices) {
    ...
    // Total general - CONDITIONNEL selon shouldShowPrices
    if (shouldShowPrices) {
...
module.exports = {
  ...
  WorkOrderEmailService
      ...
      // Total ligne - CONDITIONNEL selon shouldShowPrices  
      if (shouldShowPrices) {
