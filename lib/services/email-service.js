...
// ============================================
// SERVICE DE GÉNÉRATION PDF AMÉLIORÉ
// ============================================

class WorkOrderPDFService {
  
  // Fonction pour formater les dates en fuseau Québec
  ...
  addHeader(doc, workOrder) {
    // Layout 3 colonnes comme dans votre exemple
    
    // Colonne 1: Logo Services TMT
    try {
      doc.addImage('https://raw.githubusercontent.com/MartinDominique/gestion-bons-achat/f-feature_BT_stmt/public/logo.png', 'PNG', 20, 15, 40, 20);
    } catch (error) {
    ...
    // Colonne 2: Informations entreprise (centre)
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    ...
    doc.text(COMPANY_CITY, 105, 32, { align: 'center' });
    doc.text(`Tél: ${COMPANY_PHONE}`, 105, 37, { align: 'center' });
    doc.text(`Email: ${COMPANY_EMAIL}`, 105, 42, { align: 'center' });
    
    // Colonne 3: Information Document (droite)
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('BON DE TRAVAIL', 190, 20, { align: 'right' });
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`N°: ${workOrder.bt_number}`, 190, 30, { align: 'right' });
    doc.text(`Date: ${this.formatQuebecDateTime(workOrder.work_date)}`, 190, 37, { align: 'right' });
  ...
  addClientInfo(doc, workOrder, y) {
    ...
    if (workOrder.client.phone) {
      doc.text(`Téléphone: ${workOrder.client.phone}`, 20, y + 26);
    }
    if (workOrder.client.email) {
      doc.text(`Email: ${workOrder.client.email}`, 20, y + 34);
    }
  ...
  addWorkDetails(doc, workOrder, y) {
    ...
    // AFFICHAGE HEURES SIMPLIFIÉ - Seulement le total
    doc.text(`Heures: ${workOrder.total_hours || 0}h`, 20, y + 10);
    // PLUS DE PAUSE AFFICHÉE - retiré complètement
  ...
  addMaterials(doc, workOrder, y) {
    ...
    const tableY = y + 10;
    const rowHeight = 8;
    const colWidths = [25, 95, 25, 25]; // Code, Description, Qté, Unité
    let currentX = 20;
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text('Code', 25, tableY + 5);
    doc.text('Description', 50, tableY + 5);
    doc.text('Qté', 150, tableY + 5);
    doc.text('Unité', 170, tableY + 5);
    ...
    workOrder.materials.forEach((material, index) => {
      ...
      // Fond alternatif pour lisibilité
      if (index % 2 === 1) {
        doc.setFillColor(250, 250, 250);
        doc.rect(20, currentRowY, 170, rowHeight, 'F');
      }
      
      const productCode = material.product?.product_id || material.product_id || '';
      const productName = material.product?.description || 'Matériau sans nom';
      const quantity = material.quantity.toString();
      const unit = material.unit || material.product?.unit || 'pcs';
      
      // Texte dans les cellules
      doc.text(productCode, 25, currentRowY + 5);
      
      // Description avec wrap si nécessaire
      const descLines = doc.splitTextToSize(productName, 85);
      doc.text(descLines, 50, currentRowY + 5);
      
      doc.text(quantity, 155, currentRowY + 5, { align: 'center' });
      doc.text(unit, 175, currentRowY + 5, { align: 'center' });
      
      // Bordures cellules
      doc.rect(20, currentRowY, 170, rowHeight);
      
      currentRowY += rowHeight;
      
      // Notes matériau si présentes
      if (material.notes) {
        doc.setFontSize(8);
  ...
  addSignature(doc, workOrder, y) {
    if (workOrder.signature_data) {
      ...
      try {
        ...
        doc.setLineWidth(1);
        doc.rect(20, y + 5, 100, 40);
        
        // Ajouter l'image signature
        doc.addImage(workOrder.signature_data, 'PNG', 22, y + 7, 96, 36);
        
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        if (workOrder.client_signature_name) {
          doc.text(`Nom: ${workOrder.client_signature_name}`, 20, y + 52);
        }
        if (workOrder.signature_timestamp) {
          // DATE CORRIGÉE EN FUSEAU QUÉBEC SANS SECONDES
          const signDate = this.formatQuebecDateTime(workOrder.signature_timestamp, true);
          doc.text(`Date: ${signDate}`, 20, y + 59);
        }
  ...
  addFooter(doc) {
    ...
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    // PLUS DE "Généré le..." - seulement le numéro de page
    doc.text('Page 1', 190, pageHeight - 10, { align: 'right' });
  }

  generatePDF(workOrder) {
    try {
      console.log('Création PDF avec design amélioré...');
      const doc = new jsPDF();
...
// ============================================
// SERVICE D'ENVOI EMAIL AMÉLIORÉ
// ============================================
...
<body>
    <div class="header">
        <div class="logo">
            <!-- Placeholder pour logo - vous pouvez ajouter <img src="..."> ici -->
            <h2 style="color: #2c5aa0; margin: 0;">${COMPANY_NAME}</h2>
        </div>
        <h2 style="margin: 10px 0;">Bon de Travail ${workOrder.bt_number}</h2>
  ...
  async sendWorkOrderEmail(workOrder, options = {}) {
    try {
      ...
      // NOM DE FICHIER CORRIGÉ - Format simple : BT-2025-015.pdf
