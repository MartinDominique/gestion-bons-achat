...
class WorkOrderPDFService {
  ...
  addMaterials(doc, workOrder, y) {
    ...
    // Tableau avec bordures - AVEC PRIX SI show_prices = true
    const tableY = y + 10;
    const rowHeight = 8;
    
    // Colonnes dynamiques selon show_prices
    let colWidths, headers;
    if (workOrder.show_prices) {
      colWidths = [20, 70, 20, 20, 25, 25]; // Code, Description, Qté, Unité, Prix, Total
      headers = ['Code', 'Description', 'Qté', 'Unité', 'Prix Unit.', 'Total'];
    } else {
      colWidths = [25, 95, 25, 25]; // Code, Description, Qté, Unité
