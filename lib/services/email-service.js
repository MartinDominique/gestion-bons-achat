  ...
  addClientAndWorkSections(doc, workOrder, yStart) {
    ...
    if (workOrder.start_time && workOrder.end_time) {
      ...
      if (workOrder.total_hours) {
        doc.setFont('helvetica', 'bold');
        const hours = Math.floor(workOrder.total_hours);
        const minutes = Math.round((workOrder.total_hours - hours) * 60);
        doc.text('Total: ' + hours + 'h' + (minutes > 0 ? ' ' + minutes + 'min' : ''), 110, yRight);
        doc.setFont('helvetica', 'normal');
        yRight += 5;
      }
    }
