// ============================================
// SERVICE EMAIL + PDF AMÉLIORÉ - DESIGN TMT
...
// ============================================
// SERVICE DE GÉNÉRATION PDF AMÉLIORÉ - DESIGN TMT
// ============================================
...
class WorkOrderPDFService {
  ...
  addHeader(doc, workOrder) {
    // NOUVEAU DESIGN : Logo + Info compagnie à gauche, Document à droite
    
    // Logo à gauche (position fixe)
    try {
    ...
    // Informations entreprise à gauche (sous le logo)
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(COMPANY_NAME, 20, 45);
    
    doc.setFontSize(9);Tél: ${COMPANY_PHONE}
    doc.setFont('helvetica', 'normal');
    doc.text(COMPANY_ADDRESS, 20, 52);
    doc.text(COMPANY_CITY, 20, 57);
    doc.text(`Tél: ${COMPANY_PHONE}`, 20, 62);
    doc.text(`${COMPANY_EMAIL}`, 20, 67);
    
    // Information Document à droite (comme votre soumission)
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`N°: ${workOrder.bt_number}`, 190, 35, { align: 'right' });
  ...
  addClientInfo(doc, workOrder, y) {
    ...
    // DESIGN COMPACT : 2 colonnes côte à côte
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    ...
    // Colonne 2 (droite) - À partir de x=110
  ...
  addMaterials(doc, workOrder, y) {
    ...
    const colWidths = [20, 70, 20, 20, 25, 25]; // Code, Description, Qté, Unité, Prix, Total
    const headers = ['Code', 'Description', 'Qté', 'Unité', 'Prix Unit.', 'Total'];
    
    const totalWidth = colWidths.reduce((sum, width) => sum + width, 0);
    
    // En-têtes avec fond ORANGE (comme votre design)
    ...
    workOrder.materials.forEach((material, index) => {
      ...
      const productName = material.product?.description || material.description || 'Matériau sans nom';
      ...
      doc.text(descLines[0] || '', currentX + 3, currentRowY + 5); // Première ligne seulement
      ...
      // Prix unitaire - TOUJOURS AFFICHÉ dans le PDF
      doc.text(`${unitPrice.toFixed(2)} // ============================================
// SERVICE EMAIL + PDF AMÉLIORÉ - DESIGN TMT
// Fichier: lib/services/email-service.js
// ============================================
...
class WorkOrderPDFService {
  ...
  addMaterials(doc, workOrder, y) {
    ...
    // Tableau avec bordures - TOUJOURS AVEC PRIX dans le PDF
    const tableY = y + 10;
    const rowHeight = 8;
    
    // Colonnes avec prix (comme vos images)
    const colWidths = [20, 70, 20, 20, 25, 25]; // Code, Description, Qté, Unité, Prix, Total
    const headers = ['Code', 'Description', 'Qté', 'Unité', 'Prix Unit.', 'Total'];
...
, currentX + colWidths[4] - 3, currentRowY + 5, { align: 'right' });
      currentX += colWidths[4];
      
      // Total ligne - TOUJOURS AFFICHÉ dans le PDF
      doc.text(`${lineTotal.toFixed(2)} // ============================================
// SERVICE EMAIL + PDF AMÉLIORÉ - DESIGN TMT
// Fichier: lib/services/email-service.js
// ============================================
...
, currentX + colWidths[5] - 3, currentRowY + 5, { align: 'right' });
    ...
    // Total général - CONDITIONNEL selon show_prices
  ...
  addSignature(doc, workOrder, y) {
    if (workOrder.signature_data) {
      ...
      try {
        ...
        // Ajouter l'image signature - dimensions réduites
  ...
  generatePDF(workOrder) {
    try {
      console.log('Création PDF avec design TMT amélioré...');
      const doc = new jsPDF();
      
      // Construction du PDF
      this.addHeader(doc, workOrder);
      let currentY = 85; // Départ plus bas à cause du nouveau header
      ...
      console.log('PDF généré avec succès - Design TMT');
...
// ============================================
// SERVICE D'ENVOI EMAIL AMÉLIORÉ (inchangé)
// ============================================
