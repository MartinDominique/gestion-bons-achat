...
/**
 * Ventile une facture en matériel / M.O. / déplacement / forfait-autre à partir
 * de ses line_items. Le bucket « forfait/autre » absorbe le solde (forfait, lignes
 * « autre » et tout type non reconnu) pour garantir mat+mo+transport+forfait = sous-total.
 */
function salesBreakdown(inv) {
  const items = Array.isArray(inv.line_items) ? inv.line_items : [];
  const subtotal = parseFloat(inv.subtotal) || 0;

  let mat = 0, labor = 0, transport = 0;
  if (items.length > 0) {
    items.forEach((li) => {
      const t = parseFloat(li.total) || 0;
      if (li.type === 'material') mat += t;
      else if (li.type === 'labor') labor += t;
      else if (li.type === 'transport') transport += t;
      // forfait / other / inconnu → absorbé par le solde ci-dessous
    });
  } else {
    // Fallback: anciennes factures sans line_items → colonnes stockées
    mat = parseFloat(inv.total_materials) || 0;
    labor = parseFloat(inv.total_labor) || 0;
    transport = parseFloat(inv.total_transport) || 0;
  }

  mat = round2(mat);
  labor = round2(labor);
  transport = round2(transport);
  const forfait = round2(subtotal - mat - labor - transport);

  return { mat, labor, transport, forfait };
}
...
async function buildSalesReport(searchParams) {
  ...
  const { data: invoices, error } = await supabaseAdmin
    ...
    .select(`
      ...
      total_materials, total_labor, total_transport, status, is_prix_jobe, line_items
    `)
  ...
  // Ventiler chaque facture depuis ses line_items (réconcilie avec le sous-total)
  const data = (invoices || []).map((inv) => {
    const b = salesBreakdown(inv);
    const { line_items, ...rest } = inv; // on n'expose pas les line_items dans le rapport
    return {
      ...rest,
      total_materials: b.mat,
      total_labor: b.labor,
      total_transport: b.transport,
      total_forfait_other: b.forfait,
    };
  });

  const totals = data.reduce((acc, inv) => {
    acc.total_materials += inv.total_materials || 0;
    acc.total_labor += inv.total_labor || 0;
    acc.total_transport += inv.total_transport || 0;
    acc.total_forfait_other += inv.total_forfait_other || 0;
    ...
    total_materials: 0, total_labor: 0, total_transport: 0, total_forfait_other: 0,
...
module.exports = { buildSalesReport, buildPaymentsReport, salesBreakdown };
