 ...
 * @date 2026-09-14
 * @changelog
 *   1.2.0 - loadInvoiceBalance(): solde restant robuste (paiements détaillés OU crédit
 *           historique amount_paid, le plus élevé) partagé par le garde-fou anti-double
 *           paiement de l'API; lecture facture via maybeSingle + message d'erreur détaillé
 *   1.1.0 - Notes de crédit (factures à total négatif): statut « payée » quand le solde
 *           créditeur est ramené à zéro (remboursement/application), « partielle » sinon
 ...
 *
 * Note de crédit (total négatif): le solde est négatif et le « paiement » l'est aussi
 * (remboursement au client ou application sur une autre facture). Elle est donc
 * considérée réglée quand son solde revient à zéro, pas quand il devient négatif.
 *
...
async function recomputeInvoiceStatus(supabaseAdmin, invoiceId) {
  ...
  if (invErr) {
    throw new Error(`Lecture de la facture impossible pour le recalcul du paiement (${invErr.message})`);
  }
  if (!invoice) {
  ...
  const credited = sumCredited(payments);
  ...
  let status;
  let paid_at = null;
  if (Math.abs(balance) <= EPSILON && Math.abs(total) > EPSILON) {
    // Facture réglée (ou note de crédit remboursée/appliquée en totalité)
    status = 'paid';
    paid_at = new Date().toISOString();
  } else if (!isCredit && balance <= EPSILON && total > 0) {
    // Surpaiement d'une facture ordinaire: considérée réglée
    status = 'paid';
    paid_at = new Date().toISOString();
  } else if (Math.abs(credited) > EPSILON) {
    status = 'partial';
  } else {
...
/**
 * Somme des montants crédités (encaissé + escompte) d'une liste de paiements.
 * @param {Array<{amount:number, discount_applied:number}>} payments
 * @returns {number}
 */
function sumCredited(payments) {
...
/**
 * Charge une facture avec son solde RESTANT réel, calculé comme dans l'état de compte:
 * crédit = paiements détaillés (invoice_payments) OU crédit historique (amount_paid
 * rempli par le backfill, sans lignes de paiement), le plus élevé des deux.
 *
 * Robuste même si un recalcul précédent a échoué (amount_paid pas à jour): ce sont
 * les lignes de paiement qui font foi. Sert de garde-fou contre le double
 * enregistrement d'un paiement.
 *
 * @param {object} supabaseAdmin
 * @param {number|string} invoiceId
 * @returns {Promise<null|{invoice:object, payments:Array, credited:number, balance:number, isCredit:boolean}>}
 *          null si la facture n'existe pas
 */
async function loadInvoiceBalance(supabaseAdmin, invoiceId) {
  const id = parseInt(invoiceId);

  const { data: invoice, error: invErr } = await supabaseAdmin
    .from('invoices')
    .select('id, client_id, total, amount_paid, invoice_number, status')
    .eq('id', id)
    .maybeSingle();

  if (invErr) {
    throw new Error(`Lecture de la facture impossible (${invErr.message})`);
  }
  if (!invoice) return null;

  const { data: payments, error: payErr } = await supabaseAdmin
    .from('invoice_payments')
    .select('id, amount, discount_applied, payment_date')
    .eq('invoice_id', id);
  ...
  const total = Number(invoice.total) || 0;
  const isCredit = total < -EPSILON;
  const detailed = sumCredited(payments);
  const stored = Number(invoice.amount_paid) || 0;
  // Même logique que statement-data.js: le crédit historique (amount_paid sans lignes
  // de paiement) reste acquis; sur une note de crédit le règlement est négatif.
  const credited = isCredit ? Math.min(detailed, stored) : Math.max(detailed, stored);
