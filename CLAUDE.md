...
- ~~Items « À Commander » non retirés de la liste après création de l'AF~~ → Corrigé (2026-08-06)
  ...
  - Fichiers: `app/api/items-to-order/mark-ordered/route.js` v1.2.0, `components/SupplierPurchaseHooks.js` (alerte enrichie du détail). Aucune migration requise.
  - **Nettoyage optionnel recommandé:** aligner le type de la colonne — `ALTER TABLE items_to_order ALTER COLUMN supplier_purchase_id TYPE TEXT USING supplier_purchase_id::text;` (ou UUID si `supplier_purchases.id` est bien un UUID) — pour ré-activer le lien AF cliquable. Non requis pour le retrait des items.
