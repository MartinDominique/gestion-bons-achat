...
- ~~Inventaire: « En commande » reste à +0 après avoir passé un AF~~ → Corrigé (2026-08-07)
  ...
  - Correctif: (1) la requête « En commande » inclut désormais `in_order` + `ordered` + `partial`; (2) « Imprimer et envoyer au fournisseur » fait passer un AF `draft` → `ordered` (commande placée), donc il compte immédiatement en « En commande ».
  - Fichiers: `app/api/inventory/reservations/route.js` v1.1.0, `components/SupplierPurchaseHooks.js` (`savePurchaseOnly(statusOverride)`), `components/SupplierPurchaseForms.js` (`imprimerEtEnvoyerFournisseur` bump draft→ordered). Aucune migration requise.
- ~~Changement de prix (coûtant/vendant) à la commande d'un AF: historique de prix vide + aucune date~~ → Corrigé (2026-08-07)
  - Symptôme: en modifiant le coûtant/vendant dans le formulaire AF (modal « mise à jour prix inventaire »), l'onglet « Hist. Prix » restait vide (« Aucun historique ») et aucune date de changement n'apparaissait.
  - Cause: `updateInventoryPrice` (SupplierPurchaseHooks) écrasait `cost_price`/`selling_price` directement, sans passer par `buildPriceShiftUpdates` — l'ancien prix n'était donc jamais décalé dans `_1st/_2nd/_3rd`. De plus, aucune colonne de date n'existait.
  - Correctif: `updateInventoryPrice` recharge le produit courant et applique `buildPriceShiftUpdates` (décalage historique + horodatage). `priceShift.js` v1.1.0 renseigne `price_updated_at` dès qu'un prix change. La date s'affiche dans « Hist. Prix ». S'applique aussi automatiquement aux réceptions (AF/directe) et à l'édition inventaire, qui utilisaient déjà `buildPriceShiftUpdates`.
  - Fichiers: `lib/utils/priceShift.js` v1.1.0, `components/SupplierPurchaseHooks.js` (`updateInventoryPrice` + import), `components/InventoryManager.js` v3.11.0 (affichage date), `supabase/migrations/20260807_add_price_updated_at.sql`. **Reste:** exécuter la migration SQL `20260807_add_price_updated_at.sql` dans Supabase Dashboard.
  - **Amélioration (2026-08-07):** dates par niveau d'historique + « En commande » de référence
    - `priceShift.js` v1.2.0 — décale aussi `price_updated_at_1st/2nd/3rd` en parallèle des prix. `InventoryManager.js` v3.12.0 — affiche la date de chaque prix précédent (n-1/n-2/n-3) dans « Hist. Prix ». Selects mis à jour (SupplierPurchaseHooks, DirectReceiptModal, SupplierReceiptModal) pour inclure les colonnes de dates.
    - `DirectReceiptModal.js` v1.7.0 — affiche la quantité « En commande » (AF) par article (réf. via `/api/inventory/reservations`) dans le cadre « Articles à ajuster/réceptionner ».
    - `supabase/migrations/20260807b_add_price_updated_at_history.sql`. **Reste:** exécuter cette migration SQL dans Supabase Dashboard.
- ~~Items « À Commander » non retirés de la liste après création de l'AF~~ → Corrigé (2026-08-06)
  ...
  - **Récidive corrigée (2026-08-11):** malgré le correctif ci-dessus, des items restaient encore dans « À Commander » après « Créer l'AF » + envoi au fournisseur, **sans aucun message**. Trois fragilités restantes dans `markOrderedForPurchase`:
    1. **Échec avalé** — sur échec du `fetch`/de la route, seulement `console.error`, aucune alerte (invisible sur tablette).
    2. **Perte des ids sur échec** — le `finally { setPendingToOrderIds([]) }` effaçait la provenance **même en cas d'échec**; donc si l'utilisateur faisait « Imprimer » (1er save qui échoue) puis « Envoyer » (2e save `isNew=false`, code-match désactivé, ids déjà vidés) → plus rien n'était marqué. Orphelin définitif.
    3. **Aucune nouvelle tentative** — un seul essai, pas de retry sur échec transitoire.
  - Correctif: `markOrderedForPurchase` fait jusqu'à **3 tentatives**; les ids de provenance ne sont effacés **que sur succès confirmé** (une nouvelle sauvegarde peut réessayer); le code-match s'active aussi quand une provenance « Créer l'AF » reste non marquée (`isNew || pendingToOrderIds.length > 0`); un échec persistant **alerte l'utilisateur** (fin de la disparition silencieuse). `resetForm` vide aussi `pendingToOrderIds` (pas de fuite entre sessions AF).
  - Fichiers: `components/SupplierPurchaseHooks.js` (`markOrderedForPurchase` + `resetForm`). Aucune migration requise.
  - **Cause serveur identifiée (2026-08-11):** avec le feedback ci-dessus activé, l'alerte a confirmé que l'appel `POST /api/items-to-order/mark-ordered` échouait réellement côté serveur (l'`UPDATE` groupé était rejeté). Le coupable le plus probable : la colonne `items_to_order.supplier_purchase_id` est déclarée `BIGINT` alors que `supplier_purchases.id` est un `UUID` → l'écriture de `supplier_purchase_id` faisait échouer **tout** l'`UPDATE` (donc même le passage `status='ordered'`), ce qui laissait l'item dans la liste.
  - Correctif (route `mark-ordered` v1.2.0, résilient + diagnostic):
    1. **Étape 1 (critique)** — `UPDATE status='ordered' + ordered_at + updated_at + supplier_purchase_number` (colonnes 100 % sûres : `supplier_purchase_number` est TEXT et c'est la seule valeur affichée dans « Commandés »). C'est CE marquage qui retire l'item — il ne peut plus être bloqué par une colonne à risque.
    2. **Étape 2 (best-effort)** — `UPDATE supplier_purchase_id` séparé ; s'il échoue (conflit de type BIGINT/UUID), on journalise sans bloquer : l'item est déjà retiré et son N° d'AF déjà stocké.
    3. Le **vrai message Postgres** est renvoyé au client et affiché dans l'alerte (`Détail: …`) — fin des échecs opaques.
  - Fichiers: `app/api/items-to-order/mark-ordered/route.js` v1.2.0, `components/SupplierPurchaseHooks.js` (alerte enrichie du détail). Aucune migration requise pour le retrait des items.
  - **Type confirmé (2026-08-12):** `information_schema` confirme `items_to_order.supplier_purchase_id` = `bigint` (int8) et `supplier_purchases.id` = `uuid` → conflit exact. Migration `20260812_fix_items_to_order_supplier_purchase_id_type.sql` créée: `ALTER COLUMN supplier_purchase_id TYPE uuid USING NULL::uuid` (toutes les valeurs étaient NULL, les écritures bigint échouaient). Ré-active le lien AF cliquable dans la vue « Commandés ». **Reste:** exécuter cette migration dans Supabase Dashboard (optionnelle — non requise pour le retrait des items).
