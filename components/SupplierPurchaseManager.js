...
const generatePurchasePDF = (purchase) => {
  ...
  // Logo en haut à gauche (simulé avec texte pour l'instant)
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Services TMT', margin, margin + 10);
  
  // Titre "BON DE COMMANDE" en haut à droite
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text('BON DE COMMANDE', pageWidth - margin - 50, margin + 10);
  
  // Numéro du bon de commande
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text(purchase.purchase_number, pageWidth - margin - 30, margin + 20);
  doc.text(`Date: ${new Date(purchase.created_at).toLocaleDateString('fr-CA')}`, pageWidth - margin - 40, margin + 28);
  
  // Référence soumission
  if (purchase.supplier_quote_reference) {
    doc.text(`Soumission: ${purchase.supplier_quote_reference}`, pageWidth - margin - 45, margin + 36);
  } else {
    doc.text('Soumission: n/a', pageWidth - margin - 35, margin + 36);
  }
  
  // Date de livraison
  if (purchase.delivery_date) {
    doc.text(`Livraison prévue: ${new Date(purchase.delivery_date).toLocaleDateString('fr-CA')}`, pageWidth - margin - 50, margin + 44);
  }
  
  // Section Fournisseur (gauche)
  let currentY = margin + 60;
  doc.setFont('helvetica', 'bold');
  doc.text('Fournisseur:', margin, currentY);
  ...
  if (purchase.supplier_details) {
    ...
    if (purchase.supplier_details.contact_name) {
      doc.text(`Contact: ${purchase.supplier_details.contact_name}`, margin, currentY);
      currentY += 6;
    }
    if (purchase.supplier_details.address) {
      doc.text(purchase.supplier_details.address, margin, currentY);
      currentY += 6;
    }
    const cityLine = `${purchase.supplier_details.city || ''}, ${purchase.supplier_details.province || ''} ${purchase.supplier_details.postal_code || ''}`.trim();
    if (cityLine.length > 2) {
      doc.text(cityLine, margin, currentY);
      currentY += 6;
    }
    if (purchase.supplier_details.country) {
      doc.text(purchase.supplier_details.country, margin, currentY);
      currentY += 6;
    }
    if (purchase.supplier_details.email) {
      doc.text(`Email: ${purchase.supplier_details.email}`, margin, currentY);
      currentY += 6;
    }
    if (purchase.supplier_details.phone) {
      doc.text(`Tél: ${purchase.supplier_details.phone}`, margin, currentY);
    }
  }
  
  // Section Livrer à (droite)
  currentY = margin + 60;
  const rightColumn = pageWidth / 2 + 10;
  doc.setFont('helvetica', 'bold');
  doc.text('Livrer à :', rightColumn, currentY);
  doc.line(rightColumn, currentY + 2, rightColumn + 25, currentY + 2);
  ...
  if (purchase.shipping_address_details) {
    ...
    currentY += 8;
    doc.text(purchase.shipping_address_details.address, rightColumn, currentY);
    currentY += 6;
    const cityLine = `${purchase.shipping_address_details.city}, ${purchase.shipping_address_details.province} ${purchase.shipping_address_details.postal_code}`;
    doc.text(cityLine, rightColumn, currentY);
    currentY += 6;
    doc.text(purchase.shipping_address_details.country, rightColumn, currentY);
  }
  ...
  for (let i = 0; i < headers.length; i++) {
    doc.text(headers[i], columnX[i], tableStartY);
  }
  ...
  if (purchase.items && purchase.items.length > 0) {
    purchase.items.forEach((item) => {
      ...
      doc.text(item.product_id || '-', columnX[0], rowY);
      
      // Description avec gestion des notes
      let description = item.description || '-';
      if (item.notes && item.notes.trim()) {
        description += ` (${item.notes.trim()})`;
      }
      doc.text(description, columnX[1], rowY, { maxWidth: 55 });
      
      doc.text(quantity.toString(), columnX[2], rowY, { align: 'center' });
      doc.text(item.unit || 'UN', columnX[3], rowY, { align: 'center' });
      doc.text(`$${unitPrice.toFixed(4)}`, columnX[4], rowY, { align: 'right' });
      doc.text(`$${lineTotal.toFixed(2)}`, columnX[5], rowY, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  doc.text('Sous-total:', totalsStartX, totalsStartY);
  doc.text(`$${parseFloat(purchase.subtotal || 0).toFixed(2)}`, pageWidth - margin, totalsStartY, { align: 'right' });
  
  let totalY = totalsStartY;
  
  if (parseFloat(purchase.tps || 0) > 0) {
    totalY += 6;
    doc.text('TPS (5%):', totalsStartX, totalY);
    doc.text(`$${parseFloat(purchase.tps || 0).toFixed(2)}`, pageWidth - margin, totalY, { align: 'right' });
  }
  
  if (parseFloat(purchase.tvq || 0) > 0) {
    totalY += 6;
    doc.text('TVQ (9.975%):', totalsStartX, totalY);
    doc.text(`$${parseFloat(purchase.tvq || 0).toFixed(2)}`, pageWidth - margin, totalY, { align: 'right' });
  }
  
  if (parseFloat(purchase.shipping_cost || 0) > 0) {
    totalY += 6;
    doc.text('Frais de livraison:', totalsStartX, totalY);
    doc.text(`$${parseFloat(purchase.shipping_cost || 0).toFixed(2)}`, pageWidth - margin, totalY, { align: 'right' });
  }
  
  // Ligne finale et total
  totalY += 8;
  doc.setLineWidth(1);
  doc.line(totalsStartX, totalY - 2, pageWidth - margin, totalY - 2);
  
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text('TOTAL:', totalsStartX, totalY);
  doc.text(`$${parseFloat(purchase.total_amount || 0).toFixed(2)}`, pageWidth - margin, totalY, { align: 'right' });
  ...
  if (purchase.notes && purchase.notes.trim()) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(purchase.notes, margin, notesY + 8, { maxWidth: pageWidth - 2 * margin });
  }
...
if (shouldSendEmail) {
  ...
  // Enrichir avec les données actuelles des fournisseurs et adresses
  const enrichedPurchase = { ...savedPurchase };
  
  // Ajouter les détails du fournisseur
  if (savedPurchase.supplier_id && suppliers.length > 0) {
    const supplier = suppliers.find(s => s.id === savedPurchase.supplier_id);
    if (supplier) {
      enrichedPurchase.supplier_details = supplier;
    }
  }
  ...
  // Ajouter les détails de l'adresse de livraison
  if (savedPurchase.shipping_address_id && shippingAddresses.length > 0) {
    const address = shippingAddresses.find(a => a.id === savedPurchase.shipping_address_id);
    if (address) {
      enrichedPurchase.shipping_address_details = address;
    }
  }
  
  sendEmailToDominique(enrichedPurchase)
    .then(() => {
