// Dans votre navigation.js existant
...
export default function Navigation() {
  ...
  // ... vos autres états existants

  // AJOUTEZ cette protection d'auth
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT' || !session) {
        ...
        setCurrentPage('login'); // ou votre méthode de navigation
      }
    });
  ...
  // MODIFIEZ votre fonction de navigation pour vérifier l'auth
  const navigateTo = (page) => {
    // Pages qui nécessitent une authentification
    const protectedPages = ['clients', 'soumissions', 'purchase-orders', 'dashboard'];
    
    if (protectedPages.includes(page) && !user) {
      alert('🚫 Vous devez vous connecter pour accéder à cette page');
      setCurrentPage('login'); // ou votre méthode pour aller au login
      return;
    }
    
    // Votre logique de navigation existante
    setCurrentPage(page);
  };
  ...
  if (authLoading) {
    return <div className="flex justify-center items-center h-screen">
      ...
      <p className="ml-4">Vérification...</p>
    </div>;
  }

  // Votre JSX existant continue...
}
'use client';
